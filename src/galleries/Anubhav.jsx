...
const Anubhav = () => {
    ...
    const handleSubmit = async (e) =>{
        e.preventDefault()
        const res = await getDoc(doc(db , "anubhav" , "Anubhav"))
        if(!res.exists()){
            await setDoc(doc(db , "anubhav" , "Anubhav"),{images:[]})
        }
        ...
        try { 
            ...
            await uploadTask.then(() => {
                getDownloadURL(uploadTask.snapshot.ref).then(async(downloadURL) => {
                   await updateDoc(doc(db , "anubhav" , "Anubhav"),{
                     images: arrayUnion({
    ...
    let info 
    try {
        const unsub = onSnapshot(doc(db , "anubhav" , "Anubhav"), (doc) => {
